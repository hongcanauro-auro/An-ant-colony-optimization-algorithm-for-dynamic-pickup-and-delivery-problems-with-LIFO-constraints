...
int main() {
	...
	for (instance = 1; instance <= 34; instance++) {
		...
		for (int loop = 0; loop < 20; loop++) {
			...
			int iter = 0; // 订单处理的轮数
			vector<unordered_map<string, vector<node>>> population(6);
			while (todoorderlist.size() != 0) {
				current_dealingorders.clear();
				reference_point[0] = 10000000000;
				reference_point[1] = 10000000000;
				...
				for (int i = 0; i < N; i++) {
					...
					initial_population(population[i],currentorderlist,weight[i]);
				}
				update_reference(population);
				currentproblem = select_population(population);
			//	cout<<total_cost(currentproblem)<<" "<<Total_node_count(currentproblem)<<endl;
			for(int iter = 0;iter<whole_iteration;iter++){
				vector<int>permutation(N);
				iota(permutation.begin(),permutation.end(),0);
				permutation = shuffle_vector(permutation);
				for (int i = 0; i < N; i++){
					bool type;
					if(generateRandomNumber() < delta){
						type = 1;
					}else{
						type = 0;
					}
					pair<int,int> mating_couple = mating_selection(population,type,permutation[i]);
					unordered_map<string,vector<node>> child;
					child = population[permutation[i]];
					child = crossover_v2(population[mating_couple.first],population[mating_couple.second],weight[permutation[i]]);
					child = Tabu_search(child, "fuck");
					update_reference(child);
					update_problem(child,population,permutation[i],type);
				}
				// mergesolution(solution, currentproblem);
				// cout<<total_cost(solution)<<endl;
			}
				currentproblem = select_population(population);
				mergesolution(solution, currentproblem);
		...
		//	cout<<f2_totaldistance(solution)<<" "<<total_cost(solution)<<" "<<Total_node_count(solution)<<endl;
...
int main() {
	...
	//	printsolution(startsolution);
		...
		for (int iter1 = 0; iter1 < solution_construct_interation; iter1++) {
			...
			for (int ant_order = 0; ant_order < antnum2; ant_order++) {
				unordered_map<string, vector<orderitem>> unordered_solution;
				unordered_solution = allocate_order(veichle_order_relation, order_relation, currentorderlist, eta_veichleorder_relation);
				/*for(string each:veichles){
					cout<<each<<" ";
				...
				for(int iter2 = 0;iter2<route_construct_interation;iter2++){
					...
					for(int ant_route = 0;ant_route < antnum1;ant_route++){
						//cout<<ant_route<<" ";
						unordered_map<string,vector<node>> local_solution = startsolution;
						unordered_map<string,vector<int>> currentroute_exist = node_exist;
						int last_node;
...
writeNumbersToCSV(cost_data,"D:\\sci_pap\\project_code\\moead_ts\\data2.csv");
}
}
#endif
...
// 根据矩阵进行选择，不进行车辆的选择而是通过点之间的关系进行选择，但是不包含探索部分，有可能出现车辆未分配到订单的情况出现
#if 0
int main(){
	int whole_iteration = 50;
    ...
    double node_select = 0.8;
	double node_select_eta = 1.2;
	...
	while (unfinishorderlist.size() != 0) {
		...
		unordered_map<string, int> currentorder_location;
		unordered_map<string,vector<node>> best_currentsolution = startsolution;double global_best_cost = 100000000000;unordered_map<string,vector<orderitem>> best_currentproblem;
		currentorderlist = getnewproblem(unfinishorderlist, solution,startsolution,currentorder_location,"fuck");
		if(currentorderlist.size() == 0){
			current_time += timeinterval;
			continue;
		}
	//	cout<<unfinishorderlist.size()<<endl;
        ...
        double car_tao = currentroute_eta_relation[0][0];
		cout<<1/car_tao<<endl;
		car_tao /= currentroute_nodelist.size();
			...
			for(int ant = 0;ant<constuct_ant_num;ant++){
				...
				for(int i = 0;i<veichle_num;i++){
					for(int j : select_node[i]){
						probility_of_selection[i].push_back(pow(currentroute_node_relation[current_location[i]][j],node_select)*pow(currentroute_eta_relation[current_location[i]][j],node_select_eta));
					}
				}
		...
		//	local_solution = Tabu_search(local_bestsolution3,"fuck");
			...
			/*for(int i = 0;i<currentroute_nodelist.size() + veichle_num;i++){
				for(int j = 0;j<currentroute_nodelist.size() + veichle_num;j++)
    ...
    writeNumbersToCSV(cost_data,"D:\\sci_pap\\project_code\\moead_ts\\one_pheremone_data.csv");
}
}
#endif

// 希望确定订单的顺序，而后接着按照这个顺序对路径进行构建，
// 但是目前的问题是订单之间没有合适的启发式信息素矩阵
#if 0
int main(){
	int whole_iteration = 50;
    int constuct_ant_num = 6;
    double local_update_car = 0.1;
	double local_update_order = 0.1;
	double global_update_order = 0.1;
    double global_update_car = 0.1;
    double node_select = 0.8;
	double node_select_eta = 1.2;
    double test_q_node = 0.85;
	...
	//	cout<<unfinishorderlist.size()<<endl;
		...
		vector<vector<double>> order_tao_relation(currentorderlist.size() + veichle_num,vector<double>(currentorderlist.size() + veichle_num,tao));
		vector<double> currentroute_time_relation = global_time_matrix(currentroute_nodelist);
...
// 先处理单个车辆
vector<double> initial_order_eta(vector<orderitem> temp,vector<node> nodelist){
	vector<double> matrix(temp.size(),0);
	for(int i = 0;i<temp.size();i++){
		matrix[i] = 1/Distance[location[nodelist.front().getId()]][location[temp[i].getPickupFactoryId()]];
	}
}
...
int main(){
	...
	double order_tao;
	vector<double> order_eta_matrix = initial_order_eta(unfinishorderlist,startsolution[veichles.front()]);
...
int main(){
    ...
    int constuct_ant_num = 5;
    double local_update_car = 0.1;
    double global_update_car = 0.15;
    double node_select = 1;
	double node_select_eta = 1.5;
    double test_q_node = 0.7;
	double local_search_update = 0.1;
	readtime(); readistance(); readfactory();
	for(int instance = 1;instance <= 18;instance++){
    std::string instance_veichle = "D:\\sci_pap\\project_code\\moead_ts\\benchmark\\instance_" + to_string(instance) + "\\vehicle_info.csv";
    std::string instance_order = "D:\\sci_pap\\project_code\\moead_ts\\benchmark\\instance_" + to_string(instance) + "\\" + to_string(instance) + ".csv";
    orderlist.clear();order.clear();veichles.clear();solution.clear();
	readveichle(instance_veichle); 
	...
	//	cout<<unfinishorderlist.size()<<endl;
        ...
        vector<node> currentroute_nodelist = global_node_construct(currentorderlist,order_dnode,order_pnode,exist_pickup,startsolution);
        vector<vector<double>> currentroute_node_relation = global_node_matrix(currentroute_nodelist);
        vector<vector<double>>temp_eta_relation = global_node_eta_matrix(currentorderlist,startsolution,currentroute_nodelist);
		vector<vector<vector<double>>> currentroute_eta_relation(veichle_num,vector<vector<double>>(temp_eta_relation));
		vector<double> currentroute_time_relation = global_time_matrix(currentroute_nodelist);
    //    double car_tao = temp_eta_relation[0][0]/(currentroute_nodelist.size() + veichle_num);
		double car_tao = temp_eta_relation[0][0];
		vector<int> current_exist;
		for(int i = 0;i<currentorderlist.size();i++){
			current_exist.push_back(order_pnode[currentorderlist[i].getOrderId()]);
		}
		for (int iter = 0; iter < whole_iteration; iter++) {
            unordered_map<string, vector<orderitem>> local_bestproblem1; double local_best_cost3 = 10000000000;unordered_map<string,vector<node>>local_bestsolution3;
			for(int ant = 0;ant<constuct_ant_num;ant++){
				if(iter < whole_iteration/2||ant == 0){ 	// blocked
				unordered_map<string,vector<node>> local_solution = startsolution;
				vector<int> exist = current_exist;
				vector<int> current_location(veichle_num);
				for(int i = 0;i<veichle_num;i++)
					current_location[i] = i;
				while(exist.size() != 0){
					int current_veichle = getrangenumber(0,veichle_num - 1);
					string topvalue = veichles[current_veichle];
					vector<int>select_node;vector<double>probility_of_selection;
                    if(local_solution[topvalue].back().getdeliverlist().size()!=0){
                        select_node.push_back(order_dnode[local_solution[topvalue].back().getdeliverlist().back()]);
                    }
					for(int i:exist){
					...
					for(int i:select_node){
						probility_of_selection.push_back(pow(currentroute_node_relation[current_location[current_veichle]][i],node_select)*pow(currentroute_eta_relation[current_veichle][current_location[current_veichle]][i],node_select_eta));
						/*if(currentroute_nodelist[i].getpickup() == 1){
							probility_of_selection.back() /= double(veichle_num);
						}*/
					}
					int last_node = current_location[current_veichle];
					current_location[current_veichle] = select_node[selectElement1D(probility_of_selection,test_q_node)];
					int current_car_location = current_location[current_veichle];
					currentroute_eta_relation[current_veichle][last_node][current_car_location] = (1 - local_update_car)*currentroute_eta_relation[current_veichle][last_node][current_car_location] + local_update_car*car_tao;
					if (currentroute_nodelist[current_car_location].getpickup()) {
						int templocation = current_car_location;
						exist.erase(std::remove_if(exist.begin(), exist.end(), [templocation](int value) { return value == templocation; }),exist.end());
					}
				...
				for(int i = 0;i<veichle_num;i++){
                    ...
                    while (left_orderlist.size() != 0) {
                        ...
                        current_car_location = order_dnode[left_orderlist.back()];
                        // local update
                        currentroute_eta_relation[i][last_node][current_car_location] = (1 - local_update_car)*currentroute_eta_relation[i][last_node][current_car_location] + local_update_car*car_tao;
                        PDlist_next(local_solution[topvalue], "fuck444");
                        left_orderlist.pop_back();
                    }
				}
                ...
                if(tempcost<local_best_cost3){
					...
					for(int i = 0;i<veichle_num;i++)
						...
						for(int i = 0;i<veichle_num;i++){
							for(int j : select_node[i]){
								probility_of_selection[i].push_back(pow(currentroute_node_relation[current_location[i]][j],node_select)*pow(currentroute_eta_relation[i][current_location[i]][j],node_select_eta));
								// if(currentroute_nodelist[j].getpickup() == 1){
								// 	probility_of_selection[i].back() /= double(veichle_num);
								// }
							}
						}
						pair<int,int> selected_node = selectElement(probility_of_selection,test_q_node);
						...
						string topvalue = veichles[selected_node.first];
						int current_car_location = select_node[selected_node.first][selected_node.second];
						currentroute_eta_relation[selected_node.first][last_node][current_car_location] = (1 - local_update_car)*currentroute_eta_relation[selected_node.first][last_node][current_car_location] + local_update_car*car_tao;
						if (currentroute_nodelist[current_car_location].getpickup()) {
							int templocation = current_car_location;
							exist.erase(std::remove_if(exist.begin(), exist.end(), [templocation](int value) { return value == templocation; }),exist.end());
						}
		...
		//	local_bestsolution3 = Tabu_search(local_bestsolution3,"fuck");
			vector<vector<vector<double>>> temp_relation = currentroute_eta_relation;
			/*for(int t = 0;t<veichle_num;t++){
				for(int i = 0;i<currentroute_eta_relation[t].size();i++){
					for(int j = 0;j<currentroute_eta_relation[t][i].size();j++){
						currentroute_eta_relation[t][i][j] = (1 - global_update_car)*currentroute_eta_relation[t][i][j];
					}
				}
			}*/
            ...
            for(int i = 0;i<veichle_num;i++){
                ...
                for(int j = 1;j<local_bestsolution3[veichles[i]].size();j++){
                    if(local_bestsolution3[veichles[i]][j].getpickup()){
                        currentroute_eta_relation[i][last_node][order_pnode[local_bestsolution3[veichles[i]][j].getorderid()]] = (1 - global_update_car)* temp_relation[i][last_node][order_pnode[local_bestsolution3[veichles[i]][j].getorderid()]] + global_update_car/local_best_cost3;
                        last_node = order_pnode[local_bestsolution3[veichles[i]][j].getorderid()];
                    }else{
                        currentroute_eta_relation[i][last_node][order_dnode[local_bestsolution3[veichles[i]][j].getorderid()]] = (1 - global_update_car)* temp_relation[i][last_node][order_dnode[local_bestsolution3[veichles[i]][j].getorderid()]] + global_update_car/local_best_cost3;
                        last_node = order_dnode[local_bestsolution3[veichles[i]][j].getorderid()];
                    }
                }
            }
    ...
    cost_data.push_back(total_cost(solution));
	printsolution(solution);
	cout<<Total_node_count(solution)<<" "<<total_cost(solution)<<" "<<f1_timecosuming(solution)<<" "<<f2_totaldistance(solution)<<endl;
	}
    writeNumbersToCSV(cost_data,"D:\\sci_pap\\project_code\\moead_ts\\one_pheremone_data.csv");
}
}
...
// 同样进行的是单一矩阵取消订单的分配环节但是不用分别矩阵进行筛选而是仅仅通过一个单一矩阵进行选择
#if 0
int main(){
	int whole_iteration = 60;
    int constuct_ant_num = 5;
	...
	//	cout<<unfinishorderlist.size()<<endl;
        ...
        vector<vector<double>> currentroute_node_relation = global_node_matrix(currentroute_nodelist);
        vector<vector<double>>temp_eta_relation = global_node_eta_matrix(currentorderlist,startsolution,currentroute_nodelist);
		/*for(int i = 0;i<temp_eta_relation.size();i++){
			for(int j = 0;j<temp_eta_relation[i].size();j++){
				cout<<1/temp_eta_relation[i][j]<<" ";
			}
			cout<<endl;
		}*/
		vector<vector<double>> currentroute_eta_relation(temp_eta_relation);
		vector<double> currentroute_time_relation = global_time_matrix(currentroute_nodelist);
    //    double car_tao = temp_eta_relation[0][0]/(currentroute_nodelist.size() + veichle_num);
		double car_tao = temp_eta_relation[0][0];
		vector<int> current_exist;
		...
		for (int iter = 0; iter < whole_iteration; iter++) {
            unordered_map<string, vector<orderitem>> local_bestproblem1; double local_best_cost3 = 10000000000;unordered_map<string,vector<node>>local_bestsolution3;
			for(int ant = 0;ant<constuct_ant_num;ant++){
				if(iter < whole_iteration/2||ant == 0){
				unordered_map<string,vector<node>> local_solution = startsolution;
				vector<int> exist = current_exist;
				vector<int> current_location(veichle_num);
				for(int i = 0;i<veichle_num;i++)
				...
				while(exist.size() != 0){
					...
					for(int i:select_node){
						probility_of_selection.push_back(pow(currentroute_node_relation[current_location[current_veichle]][i],node_select)*pow(currentroute_eta_relation[current_location[current_veichle]][i],node_select_eta));
					}
					int last_node = current_location[current_veichle];
					current_location[current_veichle] = select_node[selectElement1D(probility_of_selection,test_q_node)];
					int current_car_location = current_location[current_veichle];
                ...
                if(tempcost<local_best_cost3){
					...
					for(int i = 0;i<veichle_num;i++)
						...
						pair<int,int> selected_node = selectElement2(probility_of_selection,test_q_node);
						int last_node = current_location[selected_node.first];
						current_location[selected_node.first] = select_node[selected_node.first][selected_node.second];
						string topvalue = veichles[selected_node.first];
						int current_car_location = select_node[selected_node.first][selected_node.second];
            ...
            // global update	
			vector<vector<double>> temp_relation = currentroute_eta_relation;
			/*for(int t = 0;t<veichle_num;t++){
				for(int i = 0;i<currentroute_eta_relation[t].size();i++){
					for(int j = 0;j<currentroute_eta_relation[t][i].size();j++){
						currentroute_eta_relation[t][i][j] = (1 - global_update_car)*currentroute_eta_relation[t][i][j];
            ...
            for(int i = 0;i<veichle_num;i++){
                ...
                for(int j = 1;j<local_bestsolution3[veichles[i]].size();j++){
                    if(local_bestsolution3[veichles[i]][j].getpickup()){
                        currentroute_eta_relation[last_node][order_pnode[local_bestsolution3[veichles[i]][j].getorderid()]] = (1 - global_update_car)* temp_relation[last_node][order_pnode[local_bestsolution3[veichles[i]][j].getorderid()]] + global_update_car/local_best_cost3;
                        last_node = order_pnode[local_bestsolution3[veichles[i]][j].getorderid()];
                    }else{
                        currentroute_eta_relation[last_node][order_dnode[local_bestsolution3[veichles[i]][j].getorderid()]] = (1 - global_update_car)* temp_relation[last_node][order_dnode[local_bestsolution3[veichles[i]][j].getorderid()]] + global_update_car/local_best_cost3;
                        last_node = order_dnode[local_bestsolution3[veichles[i]][j].getorderid()];
                    }
                }
            }
		...
		/*for(int i = 0;i<currentroute_eta_relation.size();i++){
			for(int j = 0;j<currentroute_eta_relation[i].size();j++){
				cout<<1/currentroute_eta_relation[i][j]<<" ";
			}
			cout<<endl;
		}
		cout<<endl;*/
	   ...
	   // vector<vector<double>>eta_veichleorder_relation(veichle_num, vector<double>(currentorderlist.size(), currentroute_eta_relation[0][0]*double(currentroute_nodelist.size())/double(currentorderlist.size())));
		...
		for (int iter = 0; iter < whole_iteration; iter++) {
			...
			for(int ant = 0;ant<constuct_ant_num;ant++){
				...
				order_set[veichles[selectElement1D(temp_problity,test_q_order)]].push_back(currentorderlist[i]);
	            }
                unordered_solution = order_set;
			    for (int i = 0; i < veichle_num; i++) {
				    for (orderitem temp_order : unordered_solution[veichles[i]]) {
                ...
                for(int each_car = 0;each_car<veichle_num;each_car++){
                    ...
                    while(currentroute_exist[topvalue].size()!=0){
                        ...
                        last_node = current_car_location;
                        current_car_location = select_node[selectElement1D(probility_of_nodeselect,test_q_node)];
						currentroute_eta_relation[last_node][current_car_location] = (1 - local_update_car)*currentroute_eta_relation[last_node][current_car_location] + local_update_car*car_tao;
                        if (currentroute_nodelist[current_car_location].getpickup()) {
                            int templocation = current_car_location;
                            currentroute_exist[topvalue].erase(std::remove_if(currentroute_exist[topvalue].begin(), currentroute_exist[topvalue].end(), [templocation](int value) { return value == templocation; }),currentroute_exist[topvalue].end());
				...
				while(exist.size() != 0){
					vector<double> temp_problity; 
					int ranom_v = getrangenumber(0,veichle_num - 1);
					for(int value:exist){
						temp_problity.push_back(eta_veichleorder_relation[ranom_v][value] * pow(veichle_order_relation[ranom_v][value], order_select));
					}
					int templocation = selectElement1D(temp_problity,test_q_order);
					order_set[veichles[ranom_v]].push_back(currentorderlist[exist[templocation]]);
					templocation = exist[templocation];
					exist.erase(std::remove_if(exist.begin(), exist.end(), [templocation](int value) { return value == templocation; }),exist.end());
				}
                unordered_solution = order_set;
			    for (int i = 0; i < veichle_num; i++) {
...
// 对矩阵进行标准化处理
void global_pheromone_update(vector<vector<double>> &global_eta_matrix,double average){
	double sum = 0;
	for(int i = 0;i<global_eta_matrix.size();i++){
		for(int j = 0;j<global_eta_matrix[i].size();j++){
			sum += global_eta_matrix[i][j];
		}
	}
	average *= global_eta_matrix.size();
	for(int i = 0;i<global_eta_matrix.size();i++){
		for(int j = 0;j<global_eta_matrix[i].size();j++){
			global_eta_matrix[i][j] = (average*global_eta_matrix[i][j])/sum;
		}
	}
}

// 使用总的factory 矩阵信息素处理该问题
#if 0
int main() {
    int whole_iteration = 60;
    int constuct_ant_num = 5;
	...
	for (int i = 0; i < order.size(); i++)
		unfinishorderlist.push_back(order[i]);
	int iter = 0; // 订单处理的轮数
	vector<vector<double>> global_eta_matrix(154,vector<double>(154,1));
	while (unfinishorderlist.size() != 0) {
        vector<double> cost_num;
		//cout<<unfinishorderlist.size()<<endl;
		// orderlist 前面部分都是具有取货点和送货点，后面部分都是仅仅只含有送货结点没有取货点
        ...
        vector<vector<double>> currentroute_node_relation = global_node_matrix(currentroute_nodelist);
      // vector<vector<double>> currentroute_eta_relation = global_node_eta_matrix(startsolution,order_dnode,order_pnode,veichle_order_relation,currentorderlist,currentroute_nodelist,currentroute_node_relation);
        vector<vector<double>>temp_relation = global_node_eta_matrix(currentorderlist,startsolution,currentroute_nodelist);
		vector<double> currentroute_time_relation = global_time_matrix(currentroute_nodelist);
	   // vector<vector<double>>eta_veichleorder_relation(veichle_num, vector<double>(currentorderlist.size(), currentroute_eta_relation[0][0]*double(currentroute_nodelist.size())/double(currentorderlist.size())));
		vector<vector<double>>eta_veichleorder_relation(veichle_num, vector<double>(currentorderlist.size(), temp_relation[0][0]));
		global_pheromone_update(global_eta_matrix,temp_relation[0][0]);
		double order_tao = eta_veichleorder_relation[0][0];
        double car_tao = temp_relation[0][0];
		...
		for (int iter = 0; iter < whole_iteration; iter++) {
			   ...
			   // unordered_solution = allocate_order(veichle_order_relation, order_relation, currentorderlist, eta_veichleorder_relation);
                ...
                for(int each_car = 0;each_car<veichle_num;each_car++){
                    ...
                    while(currentroute_exist[topvalue].size()!=0){
                        ...
                        for(int i:select_node){
                            sum += pow(currentroute_node_relation[current_car_location][i],node_select)*global_eta_matrix[location[currentroute_nodelist[current_car_location].getId()]][location[currentroute_nodelist[i].getId()]]*pow(currentroute_time_relation[i],time_factor);
                            probility_of_nodeselect.push_back(pow(currentroute_node_relation[current_car_location][i],node_select)*global_eta_matrix[location[currentroute_nodelist[current_car_location].getId()]][location[currentroute_nodelist[i].getId()]]*pow(currentroute_time_relation[i],time_factor));
                        }
                        for (int i = 0; i < probility_of_nodeselect.size(); i++) {
                            probility_of_nodeselect[i] /= sum;
                        }
                        ...
                        global_eta_matrix[location[currentroute_nodelist[last_node].getId()]][location[currentroute_nodelist[current_car_location].getId()]] = (1 - local_update_car)*global_eta_matrix[location[currentroute_nodelist[last_node].getId()]][location[currentroute_nodelist[current_car_location].getId()]] + local_update_car*car_tao;
                        if (currentroute_nodelist[current_car_location].getpickup()) {
                            int templocation = current_car_location;
                            currentroute_exist[topvalue].erase(std::remove_if(currentroute_exist[topvalue].begin(), currentroute_exist[topvalue].end(), [templocation](int value) { return value == templocation; }),currentroute_exist[topvalue].end());
                        }
            ...
            for(int i = 0;i<veichle_num;i++){
                ...
                for(int j = 1;j<local_bestsolution3[veichles[i]].size();j++){
                    if(local_bestsolution3[veichles[i]][j].getpickup()){
                        global_eta_matrix[location[currentroute_nodelist[last_node].getId()]][location[currentroute_nodelist[order_dnode[local_bestsolution3[veichles[i]][j].getorderid()]].getId()]] = (1 - global_update_car)* global_eta_matrix[location[currentroute_nodelist[last_node].getId()]][location[currentroute_nodelist[order_dnode[local_bestsolution3[veichles[i]][j].getorderid()]].getId()]] + global_update_car/local_best_cost3;
                        last_node = order_pnode[local_bestsolution3[veichles[i]][j].getorderid()];
                    }else{
                        global_eta_matrix[location[currentroute_nodelist[last_node].getId()]][location[currentroute_nodelist[order_dnode[local_bestsolution3[veichles[i]][j].getorderid()]].getId()]] = (1 - global_update_car)* global_eta_matrix[location[currentroute_nodelist[last_node].getId()]][location[currentroute_nodelist[order_dnode[local_bestsolution3[veichles[i]][j].getorderid()]].getId()]] + global_update_car/local_best_cost3;
                        last_node = order_dnode[local_bestsolution3[veichles[i]][j].getorderid()];
...
// 在时间间隔当中将信息素传承进入下一个子问题当中
// 建立对应关系，对node进行提取，同时更新的时候进行同时的映射遍历



// 在映射的情况下，对二者的信息素矩阵进行更新
// node_matrix1 为需要进行改变的矩阵
// 映射也是由需要改变的矩阵映射到原来的上一个循环当中的矩阵
void pheromone_update(vector<vector<double>>& node_matrix1,vector<vector<double>> node_matrix2,vector<pair<int,int>> reflection){
	double sum1 = 0,sum2 = 0;
	if(node_matrix2.size() == 0)
	cout<<node_matrix2.size()<<endl;
	for(int i = 0;i<reflection.size();i++){
		for(int j = 0;j<reflection.size();j++){
			sum1 += node_matrix1[reflection[i].first][reflection[j].first];
			sum2 += node_matrix2[reflection[i].second][reflection[j].second];
		}
	}
	for(int i = 0;i<reflection.size();i++){
		for(int j = 0;j<reflection.size();j++){
			node_matrix1[reflection[i].first][reflection[j].first] = (sum1*node_matrix2[reflection[i].second][reflection[j].second])/sum2;
		}
	}
}

// 1是新的序列，2是旧的序列
vector<pair<int,int>> getoverlap_order(unordered_map<string,vector<node>> startsolution,unordered_map<string,vector<node>> lastsolution,
vector<node> nodelist1,vector<node> nodelist2,unordered_map<string,int> pnode1,unordered_map<string,int>pnode2,unordered_map<string,int>dnode1,unordered_map<string,int>dnode2){
	vector<pair<int,int>> result_;
	for (string each : veichles) {
		int first,second;
		for(int i = 0;i<nodelist1.size();i++){
			if(nodelist1[i].getorderid() == startsolution[each][0].getorderid() && nodelist1[i].getpickup() == startsolution[each][0].getpickup()){
				first = i;
				break;
			}
		}
		for(int i = 0;i<nodelist2.size();i++){
			if(nodelist2[i].getorderid() == startsolution[each][0].getorderid() && nodelist2[i].getpickup() == startsolution[each][0].getpickup()){
				second = i;
				break;
			}
		}
		pair temp = make_pair(first,second);
		result_.push_back(temp);
	}
	for(string each:veichles){
		bool size = 0;
		int first,second;
		for(int i = 0;i<lastsolution[each].size();i++){
			if(lastsolution[each][i].getorderid() == startsolution[each][0].getorderid() && lastsolution[each][i].getpickup() == startsolution[each][0].getpickup()){
				size = 1;
				continue;
			}else{
				if(size){
					if(lastsolution[each][i].getpickup()){
						first = pnode1[lastsolution[each][i].getorderid()];
						second = pnode2[lastsolution[each][i].getorderid()];
						pair temp = make_pair(first,second);
						result_.push_back(temp);
					}else{
						first = dnode1[lastsolution[each][i].getorderid()];
						second = dnode2[lastsolution[each][i].getorderid()];
						pair temp = make_pair(first,second);
						result_.push_back(temp);
					}
				}else{
...
#if 0
int main() {
    int whole_iteration = 720;
    int constuct_ant_num = 10;
    double local_update_order = 0.1;
    double local_update_car = 0.1;
    double global_update_order = 0.1;
    double global_update_car = 0.1;
	...
	for (int i = 0; i < order.size(); i++)
		unfinishorderlist.push_back(order[i]);
	// int iter = 0; // 订单处理的轮数
	unordered_map<string,int> last_dnode,last_pnode;
	vector<vector<double>> last_matrix;
	unordered_map<string,vector<node>> last_solution;
	vector<node> last_nodelist;
	while (unfinishorderlist.size() != 0) {
        vector<double> cost_num;
		//cout<<unfinishorderlist.size()<<endl;
		// orderlist 前面部分都是具有取货点和送货点，后面部分都是仅仅只含有送货结点没有取货点
      ...
      // vector<vector<double>> currentroute_eta_relation = global_node_eta_matrix(startsolution,order_dnode,order_pnode,veichle_order_relation,currentorderlist,currentroute_nodelist,currentroute_node_relation);
	   ...
	   // vector<vector<double>>eta_veichleorder_relation(veichle_num, vector<double>(currentorderlist.size(), currentroute_eta_relation[0][0]*double(currentroute_nodelist.size())/double(currentorderlist.size())));
		...
		double order_tao = eta_veichleorder_relation[0][0];
        double car_tao = currentroute_eta_relation[0][0];
		if(last_matrix.size() != 0){
		vector<pair<int,int>> reflection = getoverlap_order(startsolution,last_solution,currentroute_nodelist,last_nodelist,order_pnode,last_pnode,order_dnode,last_dnode);
		pheromone_update(currentroute_eta_relation,last_matrix,reflection);
		}
		for (int iter = 0; iter < whole_iteration; iter++) {
            unordered_map<string, vector<orderitem>> local_bestproblem1; double local_best_cost3 = 10000000000;unordered_map<string,vector<node>>local_bestsolution3;
			for(int ant = 0;ant<constuct_ant_num;ant++){
		...
		last_dnode = order_dnode;last_pnode = order_pnode;
		last_matrix = currentroute_eta_relation;
		last_nodelist = currentroute_nodelist;
      // 	writecost(cost_num,"D:\\sci_pap\\project_code\\moead_ts\\temp_cost.csv");
        mergesolution(solution, best_currentsolution);
        startsolution = best_currentsolution;
		last_solution = startsolution;
        current_time += timeinterval;
	}
   // writesolution("D:\\sci_pap\\project_code\\moead_ts\\check2.csv",solution);
    cost_data.push_back(total_cost(solution));
...
// 再构建一个与order的执行顺序有关的序列以均衡路径规划的时候造成的取货点能由于本省的数量大于送货点所导致的趋向于形成大的block从而导致再订单数量变大的时候导致的时间上的延迟
// 并且通过更新信息素之后由于取货点和单一送货点数量上的差异所导致的信息素更新后其的反馈调整作用并不大
#if 0
int main() {
    int whole_iteration = 50;
    int constuct_ant_num = 6;
    ...
    double global_update_order = 0.1;
    double global_update_car = 0.1;
	double local_update_order_relation = 0.1;
	double global_update_order_relation = 0.1;
    double node_select = 2;
    double order_select = 0;
    double test_q_order = 0.7;
	double test_q_orderlist = 0.7;
    double test_q_node = 0.9;
	double local_search_update = 0.2;
	double time_factor = 0.0;
	readtime(); readistance(); readfactory();
	for(int instance = 1;instance <= 1;instance++){
    std::string instance_veichle = "D:\\sci_pap\\project_code\\moead_ts\\benchmark\\instance_" + to_string(instance) + "\\vehicle_info.csv";
	...
	while (unfinishorderlist.size() != 0) {
      ...
      //  vector<vector<double>> currentroute_eta_relation = global_node_eta_matrix(startsolution,order_dnode,order_pnode,veichle_order_relation,currentorderlist,currentroute_nodelist,currentroute_node_relation);
	  	vector<vector<double>>currentroute_eta_relation = global_node_eta_matrix(currentorderlist,startsolution,currentroute_nodelist);
	  	vector<double> currentroute_time_relation = global_time_matrix(currentroute_nodelist);
		vector<vector<double>>eta_veichleorder_relation(veichle_num, vector<double>(currentorderlist.size(), currentroute_eta_relation[0][0]*double(currentroute_nodelist.size())/double(veichle_num)));
		vector<vector<double>>order_eta_matrix(currentorderlist.size() + veichle_num,vector<double>(currentorderlist.size() + veichle_num,currentroute_eta_relation[0][0]));
		double order_relation_tao = order_eta_matrix[0][0];
		double order_tao = eta_veichleorder_relation[0][0];
        double car_tao = currentroute_eta_relation[0][0];
		
		for (int iter = 0; iter < whole_iteration; iter++) {
            unordered_map<string, vector<orderitem>> local_bestproblem1; double local_best_cost3 = 10000000000;unordered_map<string,vector<node>>local_bestsolution3;
			unordered_map<string,vector<string>> local_bestordered;
			for(int ant = 0;ant<constuct_ant_num;ant++){
			    unordered_map<string, vector<orderitem>> unordered_solution;
			   // unordered_solution = allocate_order(veichle_order_relation, order_relation, currentorderlist, eta_veichleorder_relation);
                unordered_map<string, vector<orderitem>> order_set;
				...
				unordered_map<string,vector<int>> exist_order;
				for(string each_car:veichles){
					exist_order[each_car];
					for(orderitem temp_order:unordered_solution[each_car]){
						exist_order[each_car].push_back(currentorder_location[temp_order.getOrderId()]);
					}
				}
				unordered_map<string,vector<string>> ordered_solution;

				// 根据unordered_solution的分配，对每一个车辆当中的订单的执行顺序进行确定
				// currentorder_location 加上veichle_num才是对应其在矩阵当中的下标
				for(int each_car = 0;each_car<veichle_num;each_car++){
					int current_location = each_car;
					ordered_solution[veichles[each_car]];
					vector<int> patial_exist = exist_order[veichles[each_car]];
					int last_node;
					while(patial_exist.size() != 0){
						int sum = 0;vector<double> probility;
						for(int temp_exist:patial_exist){
							sum += order_eta_matrix[current_location][temp_exist + veichle_num];
							probility.push_back(order_eta_matrix[current_location][temp_exist + veichle_num]);
						}
						for(int i = 0;i<probility.size();i++)probility[i] /= sum;
						last_node = current_location;
						if(generateRandomNumber() <= test_q_orderlist){
							current_location = patial_exist[findMaxIndex(probility)];
						}else{
							current_location = patial_exist[wheel_select(generateRandomNumber(),probility,"fuck55")];
						}
						patial_exist.erase(std::remove_if(patial_exist.begin(), patial_exist.end(), [current_location](int value) { return value == current_location; }),patial_exist.end());
						ordered_solution[veichles[each_car]].push_back(currentorderlist[current_location].getOrderId());
						current_location += veichle_num;
						order_eta_matrix[last_node][current_location] =(1 - local_update_order_relation) *order_eta_matrix[last_node][current_location] + local_update_order_relation*order_relation_tao;
					}
				}
                unordered_map<string,vector<node>> local_solution = startsolution;
                unordered_map<string,vector<int>> currentroute_exist;
                ...
                for(int each_car = 0;each_car<veichle_num;each_car++){
                  vector<string> exist = ordered_solution[veichles[each_car]];
				  int current_car_location = each_car,last_node;
				  while(exist.size() != 0){
					vector<int>select_node;vector<double>probility_of_nodeselect;double sum = 0;
					if(local_solution[veichles[each_car]].back().getdeliverlist().size() != 0){
						select_node.push_back(order_dnode[local_solution[veichles[each_car]].back().getdeliverlist().back()]);
					}
					select_node.push_back(order_pnode[exist[0]]);
					for(int i:select_node){
						sum += pow(currentroute_node_relation[current_car_location][i],node_select)*currentroute_eta_relation[current_car_location][i]*pow(currentroute_time_relation[i],time_factor);
						probility_of_nodeselect.push_back(pow(currentroute_node_relation[current_car_location][i],node_select)*currentroute_eta_relation[current_car_location][i]*pow(currentroute_time_relation[i],time_factor));
					}
					...
					local_solution[veichles[each_car]].push_back(currentroute_nodelist[current_car_location]);
                	PDlist_next(local_solution[veichles[each_car]], "fuck333");
				  }
				  vector<string> left_orderlist = local_solution[veichles[each_car]].back().getdeliverlist();
				  while (left_orderlist.size() != 0) {
					last_node = current_car_location;
					if(currentroute_nodelist.size() - 1<order_dnode[left_orderlist.back()]){
						cout<<"fuck that is it"<<endl;
					...
					local_solution[veichles[each_car]].push_back(currentroute_nodelist[order_dnode[left_orderlist.back()]]);
					current_car_location = order_dnode[left_orderlist.back()];
					// local update
					currentroute_eta_relation[last_node][current_car_location] = (1 - local_update_car)*currentroute_eta_relation[last_node][current_car_location] + local_update_car*car_tao;
					PDlist_next(local_solution[veichles[each_car]], "fuck444");
					left_orderlist.pop_back();
				}
                }
                Timecaculate(local_solution);
                ...
                if(tempcost<local_best_cost3){
                    ...
                    local_best_cost3 = tempcost;
                    local_bestsolution3 = local_solution;
					local_bestordered = ordered_solution;
                }
            }
            // global update
            for (int i = 0; i < veichle_num; i++) {
			...
			for(int each_car = 0;each_car<veichle_num;each_car++){
				int last_node = each_car;
				for(int j = 0;j<local_bestordered[veichles[each_car]].size();j++){
					order_eta_matrix[last_node][currentorder_location[local_bestordered[veichles[each_car]][j]] + veichle_num] = (1 - global_update_order_relation)*order_eta_matrix[last_node][currentorder_location[local_bestordered[veichles[each_car]][j]] + veichle_num] + global_update_order_relation/local_best_cost3;
					last_node = currentorder_location[local_bestordered[veichles[each_car]][j]] + veichle_num;
				}
			}
...
// 使用订单车辆矩阵分配以及订单之间的信息矩阵分配
#if 0
int main() {
    int whole_iteration = 25;
    int constuct_ant_num = 3;
	int route_ant = 4;
    double local_update_order = 0.1;
    double local_update_car = 0.1;
    double global_update_order = 0.1;
	...
	while (unfinishorderlist.size() != 0) {
        ...
        vector<double> currentroute_time_relation = global_time_matrix(currentroute_nodelist);
		vector<vector<double>>eta_veichleorder_relation(veichle_num, vector<double>(currentorderlist.size(), currentroute_eta_relation[0][0]*double(currentroute_nodelist.size())/double(veichle_num)));
		vector<vector<double>>order_eta_matrix(currentorderlist.size() + veichle_num,vector<double>(currentorderlist.size() + veichle_num,currentroute_eta_relation[0][0]*double(currentroute_nodelist.size())/double(currentorderlist.size())));
		double order_relation_tao = order_eta_matrix[0][0];
		double order_tao = eta_veichleorder_relation[0][0];
        double car_tao = currentroute_eta_relation[0][0];
		...
		for (int iter = 0; iter < whole_iteration; iter++) {
			...
			for(int ant = 0;ant<constuct_ant_num;ant++){
			   ...
			   // unordered_solution = allocate_order(veichle_order_relation, order_relation, currentorderlist, eta_veichleorder_relation);
				...
				double local_best_cost2 = 10000000000;unordered_map<string,vector<node>>local_bestsolution2;
				unordered_map<string,vector<string>> local_bestordered1;
				for(int iter2 = 0;iter2<route_ant;iter2++){
                unordered_solution = order_set;
			    for (int i = 0; i < veichle_num; i++) {
				    for (orderitem temp_order : unordered_solution[veichles[i]]) {
					    eta_veichleorder_relation[i][currentorder_location[temp_order.getOrderId()]] = (1 - local_update_order) * eta_veichleorder_relation[i][currentorder_location[temp_order.getOrderId()]] + local_update_order * order_tao;
                ...
                Timecaculate(local_solution);
                double tempcost = total_cost(local_solution);
                if(tempcost<local_best_cost2){
                    local_best_cost2 = tempcost;
                    local_bestsolution2 = local_solution;
					local_bestordered1 = ordered_solution;
                }
				}
				for(int i = 0;i<veichle_num;i++){
               	 	int last_node = i;
                	for(int j = 1;j<local_bestsolution2[veichles[i]].size();j++){
                    	if(local_bestsolution2[veichles[i]][j].getpickup()){
                        	currentroute_eta_relation[last_node][order_pnode[local_bestsolution2[veichles[i]][j].getorderid()]] = (1 - global_update_car)* currentroute_eta_relation[last_node][order_pnode[local_bestsolution2[veichles[i]][j].getorderid()]] + global_update_car/local_best_cost2;
                        	last_node = order_pnode[local_bestsolution2[veichles[i]][j].getorderid()];
                    	}else{
                        	currentroute_eta_relation[last_node][order_dnode[local_bestsolution2[veichles[i]][j].getorderid()]] = (1 - global_update_car)* currentroute_eta_relation[last_node][order_dnode[local_bestsolution2[veichles[i]][j].getorderid()]] + global_update_car/local_best_cost2;
                        	last_node = order_dnode[local_bestsolution2[veichles[i]][j].getorderid()];
                    	}
                	}
           		}
				...
				for(int each_car = 0;each_car<veichle_num;each_car++){
					int last_node = each_car;
					for(int j = 0;j<local_bestordered1[veichles[each_car]].size();j++){
							order_eta_matrix[last_node][currentorder_location[local_bestordered1[veichles[each_car]][j]] + veichle_num] = (1 - global_update_order_relation)*order_eta_matrix[last_node][currentorder_location[local_bestordered1[veichles[each_car]][j]] + veichle_num] + global_update_order_relation/local_best_cost2;
						last_node = currentorder_location[local_bestordered1[veichles[each_car]][j]] + veichle_num;
					}
				}
				if(local_best_cost2<local_best_cost3){
					local_best_cost3 = local_best_cost2;
					local_bestproblem1 = unordered_solution;
					local_bestsolution3 = local_bestsolution2;
				}
